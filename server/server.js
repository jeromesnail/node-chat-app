...
const { isRealString, toTitleCase } = require('./utils/validation');
...
io.on('connection', socket => {
  ...
  socket.on('join', (params, callback) => {
    ...
    } else if (users.getUserByName(params.name.trim())) {
      return callback(`${params.name.trim()} is already used in "${toTitleCase(params.room.trim())}" room`);
        }
    const room = toTitleCase(params.room.trim());
